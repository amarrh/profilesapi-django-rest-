...
class ProfileViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username="davinci", password="very-strong-pw")
    ...
    def test_profile_update_by_owner(self):
        response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}), {"city": "Anchiano", "biography": "Renaissance Genius"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content), {"id": 1, "user": "davinci", "biography": "Renaissance Genius", "city": "Anchiano", "avatar": None})

    def test_profile_update_by_random_user(self):
        random_user = User.objects.create_user(username="random", password="psw123123123")
        self.client.force_authenticate(user=random_user)
        response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}), {"biography": "hacked!!"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
